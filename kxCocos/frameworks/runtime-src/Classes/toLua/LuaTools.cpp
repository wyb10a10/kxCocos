...
void LuaTools::getStructShortValueByKey(lua_State* luaState, const char* key, short& value)
{
	lua_getfield(luaState, -1, key);
	value = static_cast<short>(lua_tointeger(luaState, -1));
	lua_pop(luaState, 1);
}

void LuaTools::getStructCharValueByKey(lua_State* luaState, const char* key, char& value)
{
	lua_getfield(luaState, -1, key);
	value = static_cast<char>(lua_tointeger(luaState, -1));
	lua_pop(luaState, 1);
}
