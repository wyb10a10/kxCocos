...
CBufferData::CBufferData(const CBufferData &data)
{
    m_IsInit = data.m_IsInit;
    m_IsReadMode = data.m_IsReadMode;
    m_Offset = data.m_Offset;
    m_DataLength = data.m_DataLength;
    m_BufferSize = data.m_BufferSize;

    m_Buffer = reinterpret_cast<char*>(kxMemMgrAlocate(m_BufferSize));
...
CBufferData& CBufferData::operator=(const CBufferData &data)
{
	if(m_Buffer == data.getBuffer()
		&& m_DataLength == data.m_DataLength
		&& m_BufferSize == data.m_BufferSize)
	{
		m_IsInit = data.m_IsInit;
