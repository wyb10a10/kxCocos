...
bool CResManager::addPreloadRes(const std::string& resName, const std::string& ass, const ResLoadedCallback& callback)
{
	...
	// spine
	if ((file1Suffix == ".skel" || file1Suffix == ".json") && flieS2uffix == ".atlas")
	{
...
{
	CCLOG("CResManager::setFinishCallback");
...
spine::SkeletonAnimation* CResManager::createSpine(const std::string& resName)
{
	...
	if (suffix == ".skel" || suffix == ".json")
	{
		...
		if (NULL != data)
		{
			...
			auto atlas = pSpineLoader->getAtlas(resName);
			if (atlas)
				pAnimation->setGLProgramState(GLProgramState::getOrCreateWithGLProgramName(
					GLProgram::SHADER_NAME_POSITION_TEXTURE_COLOR_NO_MVP, reinterpret_cast<Texture2D*>(atlas->rendererObject)));
#endif
			return pAnimation;
		}
	}
...
bool CResManager::removeRes(const std::string& resName)
{
	...
	// spine
	else if (suffix == ".skel" || suffix == ".json")
	{
...
void CResManager::checkLoading(float dt)
{
	...
	CCLOG("CResManager::checkLoading start call m_FinishCallback");
	if (m_FinishCallback)
	{
	...
	CCLOG("CResManager::checkLoading call m_FinishCallback over %f", end - begin);
}
}