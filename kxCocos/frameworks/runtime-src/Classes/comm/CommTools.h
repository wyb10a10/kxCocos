...
//检查两条线段是否相交
inline bool lineIntersect(const cocos2d::Vec2& p0, const cocos2d::Vec2& p1,
	const cocos2d::Vec2& p2, const cocos2d::Vec2& p3)
{
...
//检查矩形和线段是否相交
inline bool rectIntersect(const cocos2d::Rect& rect,
	const cocos2d::Vec2& p0, const cocos2d::Vec2& p1)
{
...
// 加密数值(不能加密字符串和指针, sizeof(T)可能会有问题)
// 1. 高地位交换
// 2. 和随机数亦或
template<typename T>
T encryptValue(T num, unsigned int key)
{
    char* pNum = reinterpret_cast<char*>(&num);
    char* pKey = reinterpret_cast<char*>(&key);
    for (int i = 0; i < sizeof(T) / 2; ++i)
    {
        pNum[i] = pNum[i] ^ pNum[sizeof(T) - i - 1];
        pNum[sizeof(T) - i - 1] = pNum[i] ^ pNum[sizeof(T) - i - 1];
        pNum[i] = pNum[i] ^ pNum[sizeof(T) - i - 1];
    }

    for (int j = 0; j < sizeof(T); ++j)
    {
        pNum[j] = pNum[j] ^ pKey[3 - j % 4];
    }
...
// 解密
template<typename T>
T decodeValue(T num, unsigned int key)
{
    char* pNum = reinterpret_cast<char*>(&num);
    char* pKey = reinterpret_cast<char*>(&key);
