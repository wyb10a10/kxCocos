...
#define CHECK_RESULT_LOG(result) { \
    if (FMOD_OK != result) {            \
        CCLOG("Execute False On File %s Line %d : %s", __FILE__, __LINE__, #result); \
    }									\
}
...
void CFMODAudioEngine::setBackgroundMusicParam(const char *param, float value)
{
    if (gMusicEvent)
    {
        ...
        if (FMOD_OK != result)
        {
			CCLOG("setBackgroundMusicParam fail, param:%s, result:%d", param, result);
        }
    }
}
...
EventInstance *CFMODAudioEngine::getEventInstance(const char *eventPath)
{
    ...
    if (FMOD_OK != result)
    {
		CCLOG("getEvent %s fail!", eventPath);
        return nullptr;
    }
