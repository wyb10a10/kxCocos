...
function PopTip:ctor(params)
	...
	label:setSystemFontSize(params.size)
	label:setTextColor(params.color)

    -- 增加黑底
    local labSize = label:getContentSize()
    local bgSprite = ccui.Scale9Sprite:create(cc.rect(17, 20, 1, 1), "chat_blackbg.png")
...
function PopTip.animateStay(node, params)
end

function PopTip.animateChip(node, params)
    -- 播放动画
	if nil == params then
		params = {}
	end
    ...
    -- 增加图片
    local labSize = node.lab:getContentSize()
    local bgSize =  node.bgSprite:getContentSize()
    local sprite = ccui.Scale9Sprite:createWithSpriteFrameName(params.pic)
    local width = sprite:getContentSize().width /2
    sprite:setPosition(cc.p(bgSize.width/2 + 20, bgSize.height/2))        
    sprite:setScale(params.picScale)    
    node.bgSprite:setContentSize(cc.size(labSize.width + 10, labSize.height + 10))
    node.bgSprite:addChild(sprite)
