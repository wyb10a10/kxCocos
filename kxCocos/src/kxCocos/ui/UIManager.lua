...
local function updateUI(mode)
	...
	for idx = beginIndex, endIndex do
        ...
        if view and endIndex >= view:getLocalZOrder() then
...
local function openTopUI(topUI, zorder)
    -- print("-------openTopUI------>", topUI, zorder)
    if topUI and topUI ~= "" then
        local uiId = UIManager.UI[topUI]
        -- 没有该UI
        if not uiId then
            print("setTopUI: no topUI=", topUI)
            return
        end
        -- 查询改UI是否已经打开，有打开修改zorder，没有就直接打开
        local isNeedOpen = false
        local index = UIManager.getUIIndex(uiId)
        if index == -1 then -- 没有打开，直接打开
            isNeedOpen = true
        else -- 修改zorder
            local UIInfo = UIManager.UIStack[index]
            if UIInfo and UIInfo.UIView then
                UIInfo.UIView:setLocalZOrder((zorder or #UIManager.UIStack)+1)
            else
                isNeedOpen = true
            end
        end
...
local function closeTopUI(topUI)
    -- print("-----closeTopUI-------->", topUI)
    if topUI and topUI ~= "" then
        local uiId = UIManager.UI[topUI]
        -- 没有该UI
        if not uiId then
        ...
        local index = UIManager.getUIIndex(uiId)
        if index ~= -1 then -- 打开了，设置zorder
            local UIInfo = UIManager.UIStack[index]
            if UIInfo and UIInfo.UIView and UIInfo.zorder then
                UIInfo.UIView:setLocalZOrder(UIInfo.zorder)
            end
        end
    end
end
...
local function openUI(ui, uiNode, conf, UIInfo, ...)
    ...
    uiNode:setLocalZOrder(UIInfo.zorder or #UIManager.UIStack)
    ...
    if conf.quickClose then
        ...
        if uiBackground then
            ...
            print("init quickClose faile, get Background is nil, uiId", ui)
        end
    end

    -- 设置topUI
    openTopUI(conf.topUI, UIInfo.zorder)
    ...
    -- 执行动画
    if conf.unfold and display.width ~= 1136 then
        -- 展开, 需要偏移root节点
        uiNode:getRoot():setPositionX(uiNode._orginX + (display.width - 1136)/2)
    end
...
local function getOrCreateUI(uiId, conf, ...)
    ...
    if uiView == nil then
        ...
        if conf.unfold then
            -- 展开, 需要偏移root节点
            uiView._orginX = uiView:getRoot():getPositionX()
        end
        if conf.cache then
           uiView:retain()
...
function UIManager.open(uiId, ...)
    if UIManager.isOpening or UIManager.isClosing then
        --print("a UI is Opening or or UIManager.isClosing" .. debug.traceback())
        -- 插入待打开队列
    ...
    -- 设置UI的zorder
    local zorder = #UIManager.UIStack + 1
    -- 先构建界面栈
    local UIInfo = {
        UIID = uiId,
        zorder = zorder,
    ...
    UIManager.UIStack[zorder] = UIInfo
...
function UIManager.openQuiet(uiId, openCallback, ...)
    ...
    if conf.quickClose then
        local uiNode = getOrCreateUI(uiId, conf, table.unpack(UIInfo.UIArgs))
        local uiBackground = CsbTools.getChildFromPath(uiNode:getRoot(), "Background")
        if uiBackground then
            uiBackground:setTouchEnabled(true)
            uiBackground:setSwallowTouches(true)
    ...
    local callback = function()
        ...
        local uiView = getOrCreateUI(uiId, conf, table.unpack(UIInfo.UIArgs))
	    -- 打开UI，执行配置
        local oldState = UIManager.isBuilding
        UIManager.isBuilding = true
	    openUI(uiId, uiView, conf, UIInfo, table.unpack(UIInfo.UIArgs))
        UIManager.isBuilding = oldState
...
function UIManager.close()
	...
	local close = function()
        ...
        if preViewInfo and preViewInfo.UIView and UIManager.isTopUI(preViewInfo.UIID) then
            ...
            -- 设置topUI
            openTopUI(prevConf.topUI, preViewInfo.zorder)
        else
            uiView:onClose()
        end
...
function UIManager.saveUI(popCount, openType)
...
-- 保存当前UI栈从栈底到指定UI
function UIManager.saveToLastUI(uiId, openType)
    -- 计算出到指定UI所需弹出的UI界面数量
    ...
    UIManager.saveUI(popCount, openType)
end
...
function UIManager.loadUI()
    ...
    local callback = function ()
        for i = 1, loadCount do
            ...
            if i == loadCount and info.OpenType ~= "quiet" then
                openCallback = UIManager.open
            end
            if openCallback ~= UIManager.openQuiet then
                UIManager.isBuilding = false
